...
export default function Experience() {
  ...
  <div className="max-w-6xl mx-auto px-6 lg:px-10">
    ...
    >
      ...
      <p className="text-t2 text-base mb-14">
      ...
      {experience.map((exp, i) => {
        ...
        return (
          ...
          >
            ...
            {i !== experience.length - 1 && (
              <div
                className="absolute left-0 top-4 w-[2px]"
                style={{
            ...
            <div className="ml-10 bg-surface border border-white/[0.07] rounded-2xl p-7 glow-hover hover:border-accent/20">
              ...
              <ul className="space-y-2">
                {exp.achievements.map((a, j) => (
                  <li
                    key={j}
                    className="flex items-start gap-2.5 text-[13.5px] text-t2 leading-relaxed"
                  >
                    <span className="text-accent text-[8px] mt-1.5 shrink-0">
                      ◆
                    </span>
                    {a}
                  </li>
              ...
              <div className="flex flex-wrap gap-1.5 mt-4">
                {exp.skills.map((s) => (
                  <span
                    key={s}
                    className="font-mono text-[11px] px-2.5 py-1 bg-bg-3 border border-white/[0.07] text-t2 rounded-full"
                  >
                    {s}
                  </span>
                ))}
